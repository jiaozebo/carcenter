...
public final class CameraManager {
	...
	private byte[] mCameraBufferReuse;
	...
	public synchronized void startPreview() {
		...
		if (theCamera != null && !previewing) {
			...
			// multiplying the preview image width, height, and bytes per pixel.
			...
			mCameraBufferReuse = new byte[(int) (s.width * s.height
					* bitsPerPixel * 1.0f / 8)];
			previewing = true;
	...
	public synchronized void requestPreviewFrame(Handler handler, int message) {
		...
		if (theCamera != null && previewing) {
			...
			theCamera.addCallbackBuffer(mCameraBufferReuse);
		}
	}
